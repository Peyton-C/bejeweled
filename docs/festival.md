...
bejeweled finds the count-in using the tempo Festival publishes. The clicks land on the beat grid, they are a consistent length, and the count-in runs a whole number of bars, which together separate them from music starting on the same beat. A track whose count-in cannot be read confidently is left alone rather than guessed at, so the worst case is a track that still has its count-in, not one cut in the wrong place.

Trimming stops short of the downbeat when a stem starts before it, some songs open with a pickup a fraction of a beat early and cutting on the downbeat would clip it.
